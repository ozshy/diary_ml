...
#
dir()
m1=readRDS("dcpc_2019_merged.rds")# Read data
names(m1)
dim(m1)
...
length(unique(m5$uasid))# num resp
...
#m8 = subset(m7, select = c(uasid, Method, pi_group, Amount, In_person, Bill, Marital, Gender, Age, Work, HH_size, HH_income, Education))
m8 = subset(m7, select = c(uasid, Method, pi_group, Amount, In_person, Marital, Gender, Age, HH_size, HH_income, Education))
dim(m8)
dim(m8[complete.cases(m8),])
nrow(m8) - nrow(m8[complete.cases(m8),]) # num payments lost by removing all NA
length(unique(m8$uasid)) - length(unique(m8[complete.cases(m8),]$uasid))# resp lost
m8 = m8[complete.cases(m8),]
dim(m8)

### Section 2: Describing the data
dim(m8)# num payments
length(unique(m8$uasid))# num respondents
...
m8_resp = m8[!duplicated(m8$uasid), ]# each respondent appears only once (to generate demographic stats)
dim(m8_resp)
names(m8_resp)
#
summary(m8$Amount)
...
nrow(m8_train)
length(unique(m8_train$uasid))
nrow(m8_test)
length(unique(m8_test$uasid))
